...
shinyServer(function(input, output, session) {
  ...
  setup_twitter_oauth(
    ...
    apiSecret, 
    accessToken, 
    accessTokenSecret) # Sets up the OAuth credentials for a twitteR session
  
  #Search tweets and create a data frame
  # Clean the tweets
  TweetFrame<-function(twtList)
  { 
    #do.call function is used to call any r function 
    #as.data.frame will convert tweets object(list) to data frame
  ...
  #scan function is used to scan data into vectors/list/text files
  wordDatabase<-function()
  {
    pos.words<<-c(pos.words) #it creates a vector (c function is used to create a vector)
  ...
  #reactive function takes an input and returns output
  #Reactive expressions let you control which parts of our app update when, which prevents unnecessary computation that can slow down our application
